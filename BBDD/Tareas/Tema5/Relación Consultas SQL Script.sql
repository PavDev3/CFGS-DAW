...
CREATE TABLE ENVIOS (
  ...
  CANTIDAD INT,
  PRIMARY KEY(P, C, T),
  CONSTRAINT fk_envios_proveedor FOREIGN KEY (P) REFERENCES PROVEEDORES(P) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT fk_envios_componente FOREIGN KEY (C) REFERENCES COMPONENTES(C) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT fk_envios_articulo FOREIGN KEY (T) REFERENCES ARTICULOS(T) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB;
