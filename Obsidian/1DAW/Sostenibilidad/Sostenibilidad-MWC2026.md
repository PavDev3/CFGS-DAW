...
← [[Sostenibilidad]] | Ver también: [[Sostenibilidad-MWC-Informe]]
...
#### Código de ejemplo — Backend eficiente (serverless)
...
function AgroIQDashboard() {
  const { sensorData, irrigationStatus } = useWebSocket();
      ...
      <header>
        ...
        <div className="status">
          <span className={`indicator ${irrigationStatus.active ? 'active' : 'idle'}`}>
            {irrigationStatus.active ? 'Regando' : 'Optimo'}
          </span>
        </div>
      </header>
...
**No es sostenible por tres razones:**
...
| Opción | Descripción |
|--------|-------------|
| **Hugging Face** | Europa — modelos open-source |
| **Mistral AI** | Francia — LLMs europeos |
| **Ollama** | Modelos que corren localmente |
| **Vertex AI** | Google Europa — datos en región europea |

#### Código — IA local vs. remota
